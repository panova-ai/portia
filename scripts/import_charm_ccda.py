#!/usr/bin/env python3
"""
Import CHARM EHR C-CDA exports into Portia.

This script is used by Panova staff to import patient records exported from
CHARM EHR. It resolves practitioner context from Sentia and uses service JWT
authentication.

CHARM-specific processing includes:
- Encounter synthesis from clinical notes
- Linking Conditions/Medications to Encounters
- Building Compositions from therapy notes

Usage:
    python scripts/import_charm_ccda.py <file_path> <practitioner_email> --env dev
    python scripts/import_charm_ccda.py patient.xml john.doe@panova.health --env staging
...
def lookup_practitioner_by_email(
    ...
    """
    # First, get the list of practitioners to find the one with matching email
    # Sentia's /practitioners endpoint may have search capability
    headers = {"Authorization": f"Bearer {token}"}

    # Try the practitioners search endpoint
    response = httpx.get(
    ...
    if org_response.status_code == 200:
        ...
        # Try alternative: get organizations for current context
        organization_id = None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Import CHARM EHR C-CDA exports into Portia",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Import a CHARM C-CDA file for a practitioner in dev environment
    python scripts/import_charm_ccda.py patient.xml john.doe@panova.health --env dev

    # Import to staging environment
    python scripts/import_charm_ccda.py patient.xml jane.smith@panova.health --env staging

    # Import with explicit organization ID (skips practitioner lookup)
    python scripts/import_charm_ccda.py patient.xml --env dev \\
        --org-id 12345678-1234-1234-1234-123456789abc

    # Dry run to validate inputs
    python scripts/import_charm_ccda.py patient.xml john.doe@panova.health --env dev --dry-run
        """,
    )
    ...
    parser.add_argument(
        "file_path",
        type=Path,
        help="Path to the C-CDA XML file to import",
    )
    ...
    parser.add_argument(
        "--practitioner-id",
        dest="practitioner_id",
        help="Practitioner ID (optional, overrides email lookup)",
    )
    ...
    if args.practitioner_email and not (organization_id and practitioner_id):
        print(f"Looking up practitioner: {args.practitioner_email}...")

        # Create token for Sentia
    ...
    try:
        ...
        if result.get("resources_extracted"):
            print("  Resources extracted:")
            for resource_type, count in result["resources_extracted"].items():
                if count > 0:
